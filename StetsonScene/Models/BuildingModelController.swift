...
struct FirebaseImage: View {
    ...
    init(id: String, emc: EventModelController) {
        self.imageLoader = Loader(id, emc: emc)
        self.eventModelController = emc
    }
...
//    }
    ...
    init(_ id: String, emc: EventModelController) {
        self.eventModelController = emc
        self.eventModelController.buildingModelController.imgLoaded = false
        ...
        ref.getData(maxSize: 25 * 1024 * 1024) { data, error in
            ...
            DispatchQueue.main.async {
                self.data = data
                print("LOADED IMAGE * * * * *")
                if !self.eventModelController.buildingModelController.isKeyInUserDefaults(key: id) {
                    self.eventModelController.buildingModelController.storeImg(image: UIImage(data: data!)!, forKey: id)
                }
                self.eventModelController.buildingModelController.imgLoaded = true
            }
        }
    }
}
